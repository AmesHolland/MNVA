...
<template>
  <div class="task-visual-container">
    <div class="chart-section-title">📊 {{ task.agent_name.replace('_Agent', '') }}: {{ task.summary }}</div>

    <template v-if="task.agent_name === 'Global_Monitor_Agent'">
      <div class="chart-box large"><EchartsCanvas chartType="global_map" :chartData="task.visualization_data.geo" /></div>
      <div class="chart-box medium"><VegaLiteChart chartType="theme_river" :chartData="task.visualization_data.trend" /></div>
    </template>

    <template v-if="task.agent_name === 'Deep_Dive_Agent'">
      <div class="chart-box large"><EchartsCanvas chartType="deep_dive_map" :chartData="task.visualization_data.map_chart" /></div>
      <div class="grid-2col">
        <div class="chart-box small"><EchartsCanvas chartType="radar" :chartData="task.visualization_data.radar_chart" /></div>
        <div class="chart-box small"><VegaLiteChart chartType="scatter_timeline" :chartData="task.visualization_data.gantt_chart" /></div>
      </div>
    </template>
